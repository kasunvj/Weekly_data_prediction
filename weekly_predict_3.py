...
def create_dataset(raw_dataset, look_back):
    dataX,dataY = np.zeros((len(raw_dataset-look_back),7),float), np.zeros((len(raw_dataset-look_back),7),float)
    for i in range (len(raw_dataset)-look_back-1):
        for j in range (0,7):
            dataX[i][j] =raw_dataset[i:i+look_back,j]
            dataY[i][j] = raw_dataset[i+look_back,j]
