...
csvworkbook0= open('sampletmbz.csv','r')


#worksheet = workbook.sheet_by_index(0)
reader0 = csv.reader(csvworkbook0)

#row_count = worksheet.nrows
row_count = sum(1 for row in reader0)
csvworkbook0.close()
...
for day in range(0,7):
    ...
    csvworkbook = open('sampletmbz.csv','r')
    reader1 = csv.reader(csvworkbook)

    for pi in reader1:
        #x = worksheet.cell(i,day).value

        if (is_number(pi[day])):
            raw_dataset[count][0]= pi[day]
        else:
            raw_dataset[count][0] = 50
        count = count +1
    #print(count)
    count =0;
    print(raw_dataset)
